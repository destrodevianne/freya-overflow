...
public class SanctumOftheLordsOfDawn extends Quest
{
    ...
    public String onTalk ( L2Npc npc, L2PcInstance player)
    {
        ...
        if (npcId == LIGHTOFDAWN)
        {
            ...
            {
                return "<html><body>??? ????:<br>?????????????,???????<br>?...? ? ? ? ? ? ? ? ? ? ??? ? ? ? ? ? ? ? ? ?...<br>? ? ?,? ? <font color=\"LEVEL\">? ? ? ? ? ? ? ? ? ? ? ? ? ?</font> ? ? ? ? ?...?</body></html>";
            }
            else if (player.getTransformation().getId() != 6204)
            {
                ...
                return "<html><body>??? ????:<br>?????????????,???????<br>?...? ? ? ? ? ? ? ? ? ??? ? ? ? ? ? ? ?...?</body></html>";
            }
        }
        else if (npcId == DEVICE)
        {
            ...
            if (tmpworld instanceof HSWorld)
            {
                ...
                if (doorst == 0)
                {
                    ...
                    ExShowScreenMessage message1 = new ExShowScreenMessage(1,0,2,15,0,15,15,false,10000,1,"????????????,???????????!");  //(????)
                    player.sendPacket(message1);
                    CreatureSay cs = new CreatureSay(0,20,"","????????????,????????????!????????????,??????????????");
                    CreatureSay cs1 = new CreatureSay(0,20,"","?????????????????????????????,????????");
