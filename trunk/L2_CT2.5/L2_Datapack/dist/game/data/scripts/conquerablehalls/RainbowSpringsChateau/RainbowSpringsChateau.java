...
public class RainbowSpringsChateau extends Quest
{
	...
	{		
		...
		public void run()
		{
			for(int arenaId : ARENA_ZONES)
			{
				...
				for(L2Character cha : (Collection<L2Character>)zone.getCharactersInsideValues())
	...
	private static void castDebuffsOnEnemies(int myArena)
	{
		for(int id : ARENA_ZONES)
		{
			...
			for(L2Character plr : (Collection<L2Character>)ZoneManager.getInstance().getZoneById(id).getCharactersInside().values())
			{
