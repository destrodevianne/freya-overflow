...
public class Q10275_ContainingTheAttributePower extends Quest
{
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		{
			case AIR:
				//La chance original es de 30% pero me parece un abuso para lo que es la quest, encima no es repetible - Zephyr
				if ((st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == YANGSWORD) && ((st.getInt("cond") == 8) || (st.getInt("cond") == 10)) && (st.getQuestItemsCount(SOULPIECEAIR) < 6) && (st.getRandom(10) <= 5))
				{
			...
			case WATER:
				int cond = st.getInt("cond");
				//La chance original es de 30% pero me parece un abuso para lo que es la quest, encima no es repetible - Zephyr
				if ((st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == YINSWORD) && ((cond >= 3) || (cond <= 5)) && (st.getQuestItemsCount(SOULPIECEWATER) < 6) && (st.getRandom(10) <= 5))
				{
